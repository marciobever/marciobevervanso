...
export default function ContestsPage() {
  ...
  async function load(){ const { data } = await supabase.from('contests').select('*').order('created_at',{ascending:false}); setItems(data||[]) }
  useEffect(()=>{ load() }, [])

  async function save(){ if(form.id) await supabase.from('contests').update(form).eq('id',form.id); else await supabase.from('contests').insert(form); setForm({}); load() }
  async function del(id:string){ if(!confirm('Excluir?'))return; await supabase.from('contests').delete().eq('id',id); load() }

  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Concursos</h1>
      <div className="grid md:grid-cols-2 gap-4">
        <div className="p-3 border rounded bg-white space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <input className="border rounded px-2 py-1" placeholder="Instituição (org)" value={form.org||''} onChange={e=>setForm((f:any)=>({...f,org:e.target.value}))}/>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            ...
            <button onClick={async()=>{ const r=await fetch(`/api/images/search?q=${encodeURIComponent(form.title||form.org||'concurso')}&limit=1`); const j=await r.json(); setForm((f:any)=>({...f,image_url:j?.images?.[0]?.src||f.image_url})) }} className="px-3 py-1.5 bg-slate-900 text-white rounded text-sm">Imagem</button>
          </div>
        ...
        <div className="space-y-2">
          {items.map(i=>(
            <div key={i.id} className="border rounded bg-white p-2 grid grid-cols-[64px_1fr_auto] gap-2 items-center">
              <img src={i.image_url||'https://picsum.photos/seed/c/128/80'} className="w-16 h-16 object-cover rounded border"/>
              <div><div className="font-semibold">{i.title}</div><div className="text-xs text-slate-500">{i.org} · {i.uf} · {i.deadline||'—'}</div></div>
              <div className="flex gap-2">
